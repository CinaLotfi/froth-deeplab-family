...
# --------- CLI ---------
def parse_args():
    p = argparse.ArgumentParser(description="Train DeepLabV3+ (ResNet-152, OS=8) on froth dataset.")
    p.add_argument(
        "--epochs",
        type=int,
        default=None,
...
def main():
    ...
    # optimizer: encoder vs decoder+head
    optimizer = AdamW(
        [
            {"params": model.encoder.parameters(),            "lr": C.backbone_lr, "weight_decay": C.weight_decay},
            {"params": model.decoder.parameters(),            "lr": C.head_lr,     "weight_decay": C.weight_decay},
            {"params": model.segmentation_head.parameters(), "lr": C.head_lr,     "weight_decay": C.weight_decay},
        ]
    )
    ...
    scheduler = LambdaLR(optimizer, lr_lambda=[poly, poly, poly])
    ...
    print(f"Starting DeepLabV3+ training for {C.epochs} epoch(s) with accumulation x{C.accum_steps}…")
    t_global = time.time()
    ...
    for epoch in range(start_epoch + 1, C.epochs + 1):
        ...
        # save LAST
        last_path = C.deeplab_v3plus_out / "last.pth"
        torch.save(
            {
                "epoch": epoch,
        ...
        if val_miou > best_miou:
            ...
            best_path = C.deeplab_v3plus_out / "best_mIoU.pth"
            torch.save(
                {
                    "epoch": epoch,
                    "model_state": model.state_dict(),
    ...
    print("\nDone.")
    print(f"[DeepLabV3+] Best mIoU: {best_miou:.4f} @ epoch {best_epoch}")
    print(f"Checkpoints in: {C.deeplab_v3plus_out}")
    print(f"  last: {C.deeplab_v3plus_out / 'last.pth'}")
    print(f"  best: {C.deeplab_v3plus_out / 'best_mIoU.pth'}")
