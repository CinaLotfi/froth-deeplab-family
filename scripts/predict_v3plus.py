...
def load_finetuned_deeplab_v3plus(device: torch.device, ckpt_path: Path | None = None) -> nn.Module:
    """
    Load DeepLabV3+ (ResNet-152) and apply finetuned weights.
    """
    model = build_deeplabv3plus_model().to(device)

    if ckpt_path is None:
        ckpt_path = C.deeplab_v3plus_out / "best_mIoU.pth"
        if not ckpt_path.exists():
            ckpt_path = C.deeplab_v3plus_out / "last.pth"

    if not ckpt_path.exists():
        raise FileNotFoundError(
            f"DeepLabV3+ checkpoint not found at:\n  {ckpt_path}\n"
            f"Run training first: python -m scripts.train_v3plus"
        )
...
def predict_split(device: torch.device, split: str, argmax: bool, thr: float):
    ...
    model = load_finetuned_deeplab_v3plus(device)

    model_tag = "deeplabv3plus_resnet152"
    ...
    print("DeepLabV3+ prediction complete.")


# --------- CLI ---------
def parse_args():
    p = argparse.ArgumentParser(description="Predict froth masks with DeepLabV3+ (ResNet-152).")
    p.add_argument(
        "--split",
        type=str,
        default="val",
