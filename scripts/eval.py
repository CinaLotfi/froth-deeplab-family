...
# --------- model loading ---------
def load_deeplab(device: torch.device, ckpt_path: Path | None = None) -> nn.Module:
    """
    If a finetuned checkpoint exists, load it.
    Otherwise, evaluate ImageNet-pretrained DeepLabV3-ResNet101.
    """
    if ckpt_path is not None and ckpt_path.exists():
        chosen = ckpt_path
    else:
        best = C.deeplab_out / "best_mIoU.pth"
        last = C.deeplab_out / "last.pth"
        if best.exists():
            chosen = best
        elif last.exists():
            chosen = last
    ...
    if chosen is None:
        print(
            f"No DeepLab checkpoints found in {C.deeplab_out}.\n"
            "Evaluating ImageNet-pretrained DeepLabV3-ResNet101."
        )
        model.eval()
        return model

    print(f"Loading DeepLab checkpoint: {chosen}")
...
# --------- CLI / main ---------
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate DeepLabV3-ResNet101 on froth dataset.")
    p.add_argument(
        "--split",
        type=str,
        default="val",
...
def main():
    ...
    model = load_deeplab(device, ckpt_path=ckpt_path)
